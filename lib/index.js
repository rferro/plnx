...
var _loop = function _loop(command) {
  ...
  exports[command] = function (options, cb) {
    ...
    if (arguments.length === 1) {
      ...
      opt = _lodash2.default.cloneDeep(options);
    }
    ...
    if (is_private && !ks) return cb(command + ": options.key and options.secret are required");
    ...
    if (missing.length) return cb(command + ": options " + missing + " are required");
    ...
    (0, _request2.default)(ropt, function (err, res, data) {
      if (err) cb(err);else if (res.statusCode !== 200) cb("statusCode " + res.statusCode);else if (data && data.error) cb(data.error);else cb(null, data);
    });
  };
};
