...
var _loop = function _loop(command) {
  ...
  exp[command] = function (opt, cb) {
    ...
    if (is_private && !ks) return cb(command + ": opt.key and opt.secret are necessary");
    ...
    try {
      for (var _iterator = cfg.params[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        ...
        var required = param.slice(-1) !== "?";

        if (required && typeof opt[param] === "undefined") missing.push(param);
      }
    } catch (err) {
